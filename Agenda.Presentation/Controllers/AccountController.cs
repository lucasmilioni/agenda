        ...
        public IActionResult Password(AccountPasswordModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (usuario != null)
                    {
                        #region Gerar uma nova senha para o usuario

                         var faker = new Faker();
                         var novaSenha = $"@1Ml{faker.Internet.Password()}";

                        #endregion

                        #region Enviando um email para o usuario contendo a nova senha

                        var subject = "Recuperação de Senha - Agenda de Contatos";
                        var body = $@"
                            <h3>Olá {usuario.Nome}</h3>
                            <p>Uma nova senha foi gerada com sucesso para o seu usuario./<p>
                            <p>Acesse a agenda com a senha: <strong>{novaSenha}</strong></p>
                            <p>Você poderá utilizar o Menu 'Minha conta' para alterar sua senha.</p>
                            <br/>
                            <p>Att, <br/>Equipe Agenda de Contatos
                        ";
                        ...
                        TempData["Menssagem"] = "Uma nova senha foi gerada com sucesso, por favor verifique sua conta de email.";
                        ModelState.Clear();
                    }
                    else
                    {
